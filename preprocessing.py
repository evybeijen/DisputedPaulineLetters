...
def remove_punctuation(text, exceptions=None):
    ...
    remove_pattern = rf"[^α-ωΑ-Ω\s{exceptions_pattern}]"

    processed_text = re.sub(remove_pattern, '', text)

    return processed_text


def remove_specific_names(text, names_to_remove):
    """
    Replaces specified names in the text with '<n>'.

    Args:
        text (str): Input text.
        names_to_remove (set[str]): Set of names to remove.
...
def clean_text(text, names_to_remove=None, exceptions=None, remove_title=True):
    """
    Cleans Greek text by removing accentuation, punctuation, capitalization, excess spaces, and optionally removing 
    specified names and leading titles.

    Args:
        text (str): Input text to clean.
        names_to_remove (list[str], optional): Specific names to remove from the text. Defaults to None.
        exceptions (set[str], optional): Characters to preserve in addition to Greek letters and spaces. Defaults to None.
        remove_title (bool, optional): If True, removes uppercase words at the start of the text. Defaults to True.
    ...
    # Optional: remove title (uppercase words at start)
    if remove_title:
        cleaned_text = re.sub(r'^[Α-Ω\s]+\s', '', cleaned_text)
    ...
    if names_to_remove:
        cleaned_text = remove_specific_names(cleaned_text, names_to_remove)
...
def split_text_into_chunks(text, delimiters={"."}, max_chunk_words=100):
    ...
    """
    # Split the text into sentence units based on the given delimiters 
